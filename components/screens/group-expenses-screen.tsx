...
export function GroupExpensesScreen() {
  ...
  return (
    <div className="space-y-10 px-container-margin lg:px-gutter pt-6 font-sans animate-in fade-in duration-500">
      ...
      <section className="glass-card rounded-[28px] border border-white/[0.05] bg-[#0E152B]/40 p-6 md:p-7 relative overflow-hidden shadow-lg">
        <div className="absolute -top-10 -right-10 w-32 h-32 bg-[#0D9488]/5 blur-2xl rounded-full pointer-events-none" />
        
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-[#0E152B] border border-white/[0.04] flex items-center justify-center text-cyan shrink-0 shadow-inner">
            <UsersRound size={22} />
          </div>
        ...
        <button
          className="w-full h-[56px] bg-primary hover:brightness-105 active:scale-[0.99] border border-white/[0.1] shadow-lg shadow-primary/20 font-black font-label-caps uppercase text-xs tracking-widest text-white rounded-full mt-6 transition-all"
          type="button"
        >
          Synchronize Ledger
        </button>
      ...
      <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
        {groupExpenses.map((expense) => (
          <article key={expense.id} className="glass-card rounded-[24px] border border-white/[0.03] p-5 hover:border-cyan/20 transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 shrink-0 rounded-xl bg-[#0E152B] border border-white/[0.04] flex items-center justify-center text-cyan shadow-inner group-hover:scale-105 transition-transform">
                <Split size={20} />
              </div>
            ...
            <div className="mt-4.5 flex -space-x-2 ml-1">
              {expense.people.slice(0, 4).map((person) => (
                <div
                  key={person}
                  className="w-8 h-8 rounded-full border-2 border-[#0B1020] bg-primary flex items-center justify-center text-[10px] font-black text-white font-label-caps shadow-sm select-none"
                  title={person}
                >
