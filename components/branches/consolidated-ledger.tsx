  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 pb-12">
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
             <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
                Macro Ledger
                <span className="px-2 py-0.5 rounded bg-primary/10 border border-primary/30 text-primary text-[10px] uppercase tracking-widest font-bold">Auditable</span>
             </h1>
             <p className="text-[#C2C6D6] text-sm font-medium mt-1">Consolidated cross-branch transaction stream parsed in real-time.</p>
          </div>
          <div className="flex gap-3">
             <button className="h-11 px-4 bg-white/[0.03] border border-white/[0.08] text-[#C2C6D6] rounded-xl font-bold text-sm flex items-center gap-2 hover:text-white hover:bg-white/[0.06] transition-all">
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="bg-[#0E152B] border border-white/[0.05] rounded-2xl p-6">
             <p className="text-[11px] font-black text-[#C2C6D6] uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="bg-[#0E152B] border border-white/[0.05] rounded-2xl p-6">
             <p className="text-[11px] font-black text-[#C2C6D6] uppercase tracking-widest">Net Vector</p>
             <h3 className="text-3xl font-black text-[#4F8CFF] mt-2">${(income - expense).toLocaleString(undefined, {minimumFractionDigits:2})}</h3>
          </div>
       </div>

       {/* Record Engine */}
       <div className="bg-[#0E152B] border border-white/[0.06] rounded-[28px] overflow-hidden relative min-h-[300px] shadow-2xl">
          {loading && (
          ...
          {error ? (
             <div className="py-20 flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-xs font-bold text-[#C2C6D6] mt-1 mb-6 max-w-sm uppercase tracking-wider">{error}</p>
             ...
             <div className="py-24 flex flex-col items-center justify-center text-center px-6">
                <div className="w-20 h-20 bg-white/[0.03] border border-white/[0.08] rounded-full flex items-center justify-center mb-4"><BookOpen size={32} className="text-[#C2C6D6]" /></div>
                <h4 className="text-xl font-black text-white">Zero Transactions Detected</h4>
                <p className="text-sm text-[#C2C6D6] mt-1 max-w-xs">Macro ledger will self-populate as operations generate activity across established branches.</p>
             </div>
          ) : (
             <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                   <thead className="bg-white/[0.02] text-[10px] font-black uppercase tracking-widest text-[#C2C6D6]">
                      <tr>
                      ...
                      {safeTxs.map((t) => {
                         ...
                         return (
                            <tr key={t.id} className="hover:bg-white/[0.01] transition-colors">
                               <td className="px-6 py-4 text-xs font-medium text-[#C2C6D6]">
                               ...
                               <td className="px-6 py-4">
                                  <div className="flex items-center gap-1.5 text-xs font-bold text-white">
                                     <MapPin size={12} className="text-[#C2C6D6]" /> {t.branchName}
                                  </div>
                                  <div className="text-[10px] font-medium text-[#C2C6D6] mt-0.5">{t.account}</div>
                               </td>
                               <td className="px-6 py-4">
