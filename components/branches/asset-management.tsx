    ...
    <div className="space-y-6 animate-in fade-in duration-700">
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
             ...
             <p className="text-[#9CA3AF] text-sm font-medium mt-1">Track hardware allocation, depreciation, and localized deployment.</p>
          </div>
          <button 
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="bg-[#111827] border border-white/[0.05] rounded-2xl p-6 flex items-center gap-5">
             ...
             <div>
                <p className="text-[11px] font-black text-[#9CA3AF] uppercase tracking-wider">Inventory Count</p>
          ...
          <div className="bg-[#111827] border border-white/[0.05] rounded-2xl p-6 flex items-center gap-5">
             ...
             <div>
                <p className="text-[11px] font-black text-[#9CA3AF] uppercase tracking-wider">Cumulative Valuation</p>
          ...
          <div className="bg-[#111827] border border-white/[0.05] rounded-2xl p-6 flex items-center gap-5">
             ...
             <div>
                <p className="text-[11px] font-black text-[#9CA3AF] uppercase tracking-wider">Network Status</p>
       ...
       <div className="relative max-w-md">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] opacity-40" size={18} />
          <input 
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by asset name or serial code..."
            className="w-full h-12 bg-[#111827] border border-white/[0.06] rounded-xl pl-12 pr-4 text-sm text-white placeholder:text-[#9CA3AF]/30 outline-none focus:border-[#4F8CFF]/50 transition-all"
          />
       </div>

       {/* Table Render */}
       <div className="bg-[#111827] border border-white/[0.06] rounded-[28px] overflow-hidden shadow-2xl min-h-[300px] relative">
          ...
          {assets.length === 0 && !loading ? (
             <div className="py-24 flex flex-col items-center justify-center text-center px-6">
                <div className="w-20 h-20 bg-white/[0.03] border border-white/[0.08] rounded-3xl flex items-center justify-center mb-5 text-[#9CA3AF]">
                   <Package size={40} />
                </div>
                <h3 className="text-xl font-black text-white">Zero Enterprise Assets Registered</h3>
                <p className="text-sm text-[#9CA3AF] mt-1.5 max-w-xs font-medium">Begin cataloging laptops, vehicles, and office machinery linked to active branches.</p>
                <button onClick={() => setIsModalOpen(true)} className="mt-8 h-11 px-6 bg-[#4F8CFF] text-white font-bold text-sm rounded-xl flex items-center gap-2 shadow-lg hover:brightness-110 transition-all">
            ...
            <div className="overflow-x-auto">
               <table className="w-full text-left border-collapse text-sm">
                  <thead className="bg-white/[0.02] text-[11px] font-black uppercase tracking-widest text-[#9CA3AF]">
                     <tr>
                     ...
                     {filtered.map((asset) => (
                        <tr key={asset.id} className="group hover:bg-white/[0.01] transition-colors">
                           <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                 ...
                                 <div>
                                    ...
                                    <div className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest mt-0.5">{asset.code || 'NOCODE'} • {asset.category}</div>
                                 </div>
                              </div>
                           </td>
                           <td className="px-6 py-4">
                              <div className="flex items-center gap-1.5 text-[#F9FAFB] font-bold text-xs">
                                 <MapPin size={12} className="text-[#9CA3AF]" /> {asset.branchName}
                              </div>
                           </td>
                           <td className="px-6 py-4">
                              <div className="flex items-center gap-1.5 text-white font-medium text-xs">
                                 <User size={12} className="text-[#9CA3AF]" /> {asset.assignedTo}
                              </div>
                           </td>
                           ...
                           <td className="px-6 py-4 text-right">
                              <button className="h-8 px-3 border border-white/[0.08] rounded-lg text-[#9CA3AF] text-xs font-bold hover:text-white transition-colors">View</button>
                           </td>
                        </tr>
                     ))}
                  </tbody>
         ...
         <motion.div initial={{ scale: 0.95, y: 20, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.95, y: 20, opacity: 0 }} className="relative bg-[#0B1015] border border-white/[0.08] rounded-[32px] w-full max-w-md shadow-2xl z-10 overflow-hidden">
            <div className="p-6 border-b border-white/[0.05] flex justify-between items-center">
               <div>
                  ...
                  <p className="text-[10px] font-bold text-[#9CA3AF] tracking-widest uppercase mt-0.5">Permanent Physical Log</p>
               </div>
               <button onClick={onClose} className="text-[#9CA3AF] hover:text-white"><X size={18} /></button>
            </div>
            ...
            <form onSubmit={submit} className="p-6 space-y-4">
               ...
               <div className="space-y-1">
                  <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Descriptor Name</label>
                  <input required value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder="MacBook Pro 16-inch" className="w-full h-11 bg-[#111827] border border-white/[0.06] rounded-xl text-sm text-white px-4 outline-none focus:border-blue-500/50" />
               </div>

               <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                     <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Asset Tag / Code</label>
                     <input required value={form.code} onChange={(e) => setForm({...form, code: e.target.value})} placeholder="LP-X-101" className="w-full h-11 bg-[#111827] border border-white/[0.06] rounded-xl text-sm text-white px-4 outline-none focus:border-blue-500/50" />
                  </div>
                  <div className="space-y-1">
                     <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Category</label>
                     <select value={form.category} onChange={(e) => setForm({...form, category: e.target.value})} className="w-full h-11 bg-[#111827] border border-white/[0.06] rounded-xl text-sm text-white px-3 outline-none appearance-none">
               ...
               <div className="space-y-1">
                  <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Purchase Base Amount ($)</label>
                  <input required type="number" value={form.purchaseAmount} onChange={(e) => setForm({...form, purchaseAmount: e.target.value})} placeholder="0.00" className="w-full h-11 bg-[#111827] border border-white/[0.06] rounded-xl text-sm text-white px-4 outline-none focus:border-blue-500/50" />
               </div>

               <div className="space-y-1">
                  <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Branch Anchor</label>
                  <select required value={form.branchId} onChange={(e) => setForm({...form, branchId: e.target.value})} className="w-full h-11 bg-[#111827] border border-white/[0.06] rounded-xl text-sm text-white px-3 outline-none appearance-none">
               ...
               <div className="space-y-1">
                  <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Assigned Operator (Optional)</label>
                  <select value={form.assignedUserId} onChange={(e) => setForm({...form, assignedUserId: e.target.value})} className="w-full h-11 bg-[#111827] border border-white/[0.06] rounded-xl text-sm text-white px-3 outline-none appearance-none">
               ...
               <div className="pt-4 flex gap-3 border-t border-white/[0.05]">
                  ...
                  <button type="submit" disabled={saving || branches.length === 0} className="flex-1 h-11 bg-[#4F8CFF] text-white font-bold text-sm rounded-xl flex items-center justify-center gap-2 hover:brightness-110 disabled:opacity-50">
